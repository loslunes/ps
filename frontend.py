...
def dial(color_array, arrow_index, labels, ax):
    ...
    white_half = np.ones(len(color_array))*0.8
    color_half = color_array
    color_pallet = np.concatenate([color_half, white_half])
 
    alpha = np.concatenate([np.ones(len(color_array)), np.zeros(len(color_array))])
...
def runDial(arrow_index):
    ...
    fig, ax = plt.subplots(figsize=(10, 5))  # Adjust these values as needed
